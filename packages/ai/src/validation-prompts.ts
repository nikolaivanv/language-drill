...
// 2026-08-11 (validate@2026-08-11): mirrors generate@2026-08-11 (generate↔validate
// contract split). Polarity-determinacy sub-bullet on the `ambiguous` dimension:
// the superlative frame is SYMMETRIC, so a `más`/`menos` blank whose stem carries
// no evaluative anchor is ambiguous regardless of which pole `correctAnswer`
// names. Without this, the validator keeps approving the pattern at 0.85+ (it
// approved 12 of 14 such rows in es-b1-superlatives-comparisons) because the
...
// 2026-08-11a (validate@2026-08-11a): validator alternative-enumeration —
// adds a lead-in sub-bullet on the `ambiguous` dimension, FIRST under dimension
// 2, instructing the model to actually fill the report-only `candidateFillers`
// field (added to the cloze validation tool schema in Task 1) before deciding
// `ambiguous`: propose 2-4 distinct fillers, adjudicate each against the
// visible sentence alone (never against `correctAnswer`), and rule one out
// only with a quoted forbidding span. The existing tense- and
// polarity-determinacy sub-bullets are reframed as worked examples of this
// general procedure, not replaced. Template edit → Langfuse push per env.
// 2026-08-12 correction (still validate@2026-08-11a — not yet pushed to
// Langfuse, so amending in place is correct): the lead-in sub-bullet's
// closing clause read "absent from `acceptableAnswers`", but the tool's
// `candidateFillers` description instructs the model to list
// `correctAnswer` itself as a candidate, which is always adjudicated
// `also-correct`. Since `acceptableAnswers` is contractually the set of
// answers ADDITIONAL to `correctAnswer` (fluency.ts:90 builds the accepted
// set as `[correctAnswer, ...acceptableAnswers]`), the literal instruction
// told the model to mark `ambiguous = true` on every cloze. Fixed to
// "absent from BOTH `correctAnswer` and `acceptableAnswers`", mirroring the
// same fix applied to `applyCandidateFillerConsistency` in validate.ts.
// 2026-08-12 (from main, #639/#642): the cloze validation USER prompt now
// renders `glossEn` as a "**Meaning (shown to the learner):**" line, so the
// validator can check `acceptableAnswers` against the gloss. The accompanying
// gloss-consistency rule in VALIDATION_SYSTEM_PROMPT_TEMPLATE IS a cached-template
// edit: inert in production until `push-prompts` runs per environment.
// 2026-08-13 (validate@2026-08-13): merge of the two independent template edits
// above — the candidateFillers lead-in on the `ambiguous` dimension and main's
// gloss-consistency rule. Both survive in the template body; only this constant
// and the changelog conflicted. The version is bumped past BOTH parents
// (validate@2026-08-11a on this branch, validate@2026-08-12 on main) so Langfuse
// cohorts the merged body as its own population rather than folding it into
// either parent's traces. Template edit → Langfuse push per env.
...
// (saber/poder) mirroring the generator-side clause. Template edit →
// Langfuse push per env.
// 2026-08-13a (validate@2026-08-13a): second merge of two independently-bumped
// template edits. Both parents landed on validate@2026-08-13 — this branch's
// candidateFillers lead-in, and main's #644 (contextSpoilsAnswer naming
// `glossEn`, plus the neutral-gloss rule for lexical-choice points). The merged
...
// 2026-08-18 (validate@2026-09-21): the per-draft USER prompt now names the
...
// class, different scale point — where enumeration does NOT cure and the fix is
// a tighter definition. Mirrors the generation-side rule added in
// generate@2026-08-17; a one-sided change would be nullified (the generator
// would stop straddling while the validator kept accepting straddled drafts
// that enumerate, or vice versa).
//
// Suffixed rather than reusing 2026-08-17 because that version is ALREADY TAKEN
// by the conjugation post-answer carve-out earlier today (#656) — a different
// body. Reusing it would fold two distinct prompts into one Langfuse cohort,
// the exact mistake the 2026-08-13a note above records.
//
// Unlike #656 this IS a cached-template edit -> needs `push-prompts` per env.
// Bumped 2026-09-21 (validate@2026-09-21): mirrors generate@2026-09-21's
// lexeme-determinacy rule as an `ambiguous` sub-bullet, plus an explicit
// instruction to ignore the MC `options` array when judging a cloze. Both come
// from the same learner flag. A paired dry-run showed the pre-bump prompt
// already demoting 10 of 84 rows on the offending point for exactly this
// defect -- but MISSING both rows the learner actually hit, while flagging a
// structurally identical sibling (`no se lo cuentes` vs `digas`): the generic
// candidateFillers lead-in fires only when the judge happens to propose a
// rival lexeme, so the class needed a named sub-bullet to fire consistently,
// exactly as tense- and polarity-determinacy did. The options sub-bullet comes
// from the same run's notes, where the judge repeatedly talked itself down
// with "if this is multiple-choice, the distractors resolve the ambiguity" --
// they do not: `options` are an opt-in scaffold hidden behind a "show answer
// options" control, so every cloze must be judged as free production.
// Template edit -> Langfuse push per env.
// Bumped 2026-09-22 (validate@2026-09-22): mirrors generate@2026-09-22's
// clitic-doubling-determinacy rule as an `ambiguous` sub-bullet. A
// revalidate:cloze dry-run over the four offending rows under the PRE-bump
// prompt returned 4 no-change / 0 demoted -- neither the generic
// candidateFillers lead-in nor the freshly-added lexeme sub-bullet fires on
// this class, because the rival form is the SAME lexeme differing only by a
// clitic. Note the polarity of this one is inverted relative to its three
// siblings: tense, polarity and lexeme determinacy all say "anchor, never
...
2. **ambiguous** (boolean): more than one substantively-correct answer? For **cloze**, true when multiple lexemes/forms satisfy the grammar point in this sentence AND \`acceptableAnswers\` does not enumerate them. For **translation**, surface variation is fine; two or more structurally-different correct renderings are ambiguous ONLY when \`acceptableAnswers\` does not enumerate them — enumeration cures it exactly as for cloze (e.g. TR "In my opinion…" → both \`Bence …\` and \`Bana göre …\`; listing the alternative, or wording the source to force one structure, is NOT ambiguous). For **vocab_recall**, the prompt must pick out exactly one headword — or, when the language has true near-synonyms that the definition admits equally (e.g. TR \`istasyon\`/\`gar\` for a station definition), \`acceptableAnswers\` must enumerate every defensible alternate; enumerated near-synonyms cure the ambiguity, a missing defensible alternate does not. **Enumeration cures ONLY same-referent synonyms.** When the listed alternate denotes a DIFFERENT thing — a superordinate/subordinate pair (\`der Schreibtisch\` desk vs \`der Tisch\` any table), an adjacent-but-distinct concept (\`die Wiese\` meadow vs \`die Ebene\` plain; \`la terminal\` vs \`la estación\`), a different word class (\`Nachhaltigkeit\` noun vs \`nachhaltig\` adjective), or a different point on one scale (\`heiß\` vs \`warm\`) — listing it does NOT cure the item: a vocab_recall prompt must elicit exactly one headword, and a definition admitting two distinct referents has no single right answer. Set \`ambiguous = true\` and say which cue belongs to the neighbour. The cure is a tighter definition (delete the neighbour's cues), never a longer \`acceptableAnswers\`. For **sentence_construction**, \`ambiguous\` is about the PROMPT, not the answer space: open production legitimately has many correct sentences (different modals, lexis, word order, polarity), and that is the task design — it is NOT \`ambiguous\`, and no \`acceptableAnswers\` enumeration is expected or possible. Set \`ambiguous = true\` ONLY when the PROMPT itself is self-contradictory (e.g. it instructs "reply as \`du\`" — a register/addressee cue — while the situation requires a first-person \`ich\` answer, so no coherent single sentence satisfies it) or so under-specified the learner cannot tell what structure to produce. Never set it merely because multiple valid sentences exist, or because the model answers differ in lexis, word order, or polarity.
   ...
   - **Tense-determinacy (cloze):** when the blank is a finite verb and tense/aspect is NOT what this point drills, a NON-PRESENT \`correctAnswer\` (preterite/imperfect/perfect) is \`ambiguous = true\` UNLESS the visible sentence contains a temporal anchor that forces the past — a preterite/imperfect/pluperfect verb elsewhere in the stem (\`llegué\`, \`era\`, \`habíamos logrado\`) or an explicit past adverbial (\`ayer\`, \`anoche\`, \`esa noche\`, \`la semana pasada\`). This is same-lexeme tense ambiguity (\`deja\` vs \`dejó\`); enumeration does NOT cure it (listing both teaches they are interchangeable) — a clean draft forces the past via an in-stem anchor or targets the present/habitual. Habitual/iterative cues (\`siempre\`, \`cada vez que\`, \`todos los días\`, \`por las noches\`, \`normalmente\`, \`a menudo\`) forbid the PRETERITE (making a preterite \`correctAnswer\` \`ambiguous\`), but they do NOT force the present: they license the present-habitual AND the IMPERFECT (past-habitual) equally, so a PRESENT \`correctAnswer\` whose only time cue is a habitual/iterative adverbial is ALSO \`ambiguous = true\` — the imperfect fits identically (\`El ruido de la calle no me ___ dormir por las noches.\` / \`"deja"\` is ambiguous because \`dejaba\` is equally valid; \`El entrenador nos ___ correr diez kilómetros todos los días.\` / \`"hace"\` is ambiguous because \`hacía\` is equally valid). Such a present answer is unambiguous ONLY when the stem carries a PRESENT anchor (\`ahora\`, \`hoy en día\`, \`estos días\`, \`últimamente\`, or another present-tense verb). Do NOT flag a truly anchorless present answer (a generic statement with NO time cue at all), nor a preterite/imperfect answer that DOES carry an in-stem past anchor. Example: \`El portero no ___ entrar al mensajero sin identificación.\` / \`correctAnswer: "dejó"\` — ambiguous (present \`deja\` equally valid, no anchor); \`Cuando llegué tarde, mi jefe no me ___ disculparme.\` / \`"dejó"\` — NOT ambiguous (the preterite \`llegué\` forces the past).
   - **Polarity-determinacy (cloze):** when the blank is the POLARITY slot of a comparative/superlative (ES \`más\`/\`menos\`, with or without the article; DE \`am meisten\`/\`am wenigsten\`; TR \`en\`/\`en az\`), set \`ambiguous = true\` UNLESS the visible sentence carries an EVALUATIVE anchor that fits only one pole — a consequence, reaction, or fact about the situation (\`…y por eso lo amamos tanto\` → \`más bonito\`; \`…así que venimos aquí cada semana\` → \`más barato\`; \`…no despegamos los ojos de la pantalla\` → \`menos aburrida\`). The superlative frame alone (\`de todos\`, \`de toda la ciudad\`, \`del mundo\`, \`de todos los que hice este año\`) is SYMMETRIC and is NOT an anchor: \`El examen de matemáticas fue ___ difícil de todos los que hice este año.\` / \`correctAnswer: "el más"\` IS ambiguous, because \`el menos\` is equally valid on the visible text. This applies in BOTH directions — \`Esta película es la ___ aburrida de todas las que hemos visto este año.\` / \`"menos"\` is ambiguous for the same reason (\`más aburrida\` fits identically). Enumeration does NOT cure it: \`más\` and \`menos\` are ANTONYMS, not alternants, so listing both teaches they are interchangeable — a clean draft anchors one pole and lists neither in \`acceptableAnswers\`. Also set \`ambiguous = true\` when the blank swallows the ADJECTIVE and the stem does not entail that exact adjective (\`Este hotel es el ___ de toda la ciudad, pero también es el más cómodo.\` / \`"más caro"\` — \`más grande\`/\`más ruidoso\`/\`más lujoso\` fit equally). Separately, when an explicit head noun follows the blank so that substituting the answer yields \`la más institución importante\` (correct order: \`la institución más importante\`), the draft is malformed — set \`qualityScore\` below 0.5 rather than merely flagging it.
   - **Lexeme-determinacy (cloze):** when the blank swallows the verb's LEXICAL STEM (not just an inflection of a verb visible in the stem) and the point drills a FORM — mood, clitic placement, agreement, periphrasis — rather than which verb the meaning selects, set \`ambiguous = true\` UNLESS one of two things holds: the visible sentence ENTAILS that verb (an object, consequence, or reaction only it fits — \`espera a las rebajas\` forces \`comprar\`; \`guarda un poco para mañana\` forces \`comer\`), or the verb's citation form is given in parentheses. Apply the \`candidateFillers\` discipline literally here: a rival lexeme is \`ruled-out\` only if you can QUOTE the span that forbids it — "it is not the intended answer" and "the intended verb is more idiomatic" are not rulings. \`Tienes razón, ese problema es importante. ___ ahora, espera hasta mañana.\` / \`"No lo comentes"\` IS ambiguous (\`no lo menciones\`, \`no lo discutas\`, \`no lo hagas\` fit identically); \`Tienes los apuntes, pero no ___ a nadie.\` / \`"se los prestes"\` IS ambiguous (\`se los des\`). **Enumeration does NOT cure this one.** The rival set is an OPEN class, so an \`acceptableAnswers\` list is necessarily incomplete — a draft listing \`menciones, digas, cuentes\` is STILL \`ambiguous = true\` if the frame admits \`discutas\` or \`hagas\`; the cure is an in-stem anchor or a parenthetical infinitive, exactly as for tense- and polarity-determinacy. A parenthetical that carries the clitics or the tested inflection (\`(enviarlo)\` where the point is clitic PLACEMENT, \`(no lo envíes)\`) is \`contextSpoilsAnswer = true\`, not a cure. Do NOT apply this when the lexical choice IS the point (ser/estar, saber/poder, verbs of change) — the Form-contrast exception governs there.
   - **Clitic-doubling-determinacy (cloze):** when the blank swallows the CLITIC(S) and the visible stem carries an OVERT POSTVERBAL DATIVE phrase (\`a nadie\`, \`a alguien\`, \`a tu hermano\`, \`a los niños\`, \`a María\`), Spanish leaves the dative clitic OPTIONAL, so the cluster with and without it are BOTH correct. Set \`ambiguous = true\` UNLESS \`acceptableAnswers\` already lists the counterpart of \`correctAnswer\`. \`Esta historia de fantasía es un secreto. ___ a nadie, por favor.\` / \`"No la cuentes"\` with no \`acceptableAnswers\` IS ambiguous (\`No se la cuentes\` is equally valid); the trap is SYMMETRIC, so \`La verdad ya salió a la luz — ___ a nadie, por favor.\` / \`"No se lo digas"\` with no \`acceptableAnswers\` is ambiguous for the mirror-image reason (\`No lo digas\` is equally valid). **Enumeration DOES cure this one** — the rival set is CLOSED at exactly two members (±the dative clitic), which is why this sub-bullet differs from tense-, polarity- and lexeme-determinacy above: there the cure is an anchor and enumeration is forbidden, here a two-entry \`acceptableAnswers\` is the whole fix and a row that carries it is NOT ambiguous. Do NOT flag when the dative phrase is PREVERBAL / left-dislocated (\`A tu hermano ___ cuidan muy bien en ese hospital.\`), where doubling is obligatory; do NOT flag when the point DRILLS doubling (\`es-a2-indirect-object-pronouns-se\`), where the overt PP is the intended cue and the clitic is the task; and do NOT flag an \`a + person\` DIRECT object (\`no ___ a tu madre todavía\` / \`"la llames"\`), since accusative doubling is not free in Spanish and no extra clitic is licensed there.
   - **The MC \`options\` array does not resolve ambiguity.** Some cloze drafts carry \`options\`; in the app these are an OPT-IN scaffold, hidden behind a "show answer options" control and recorded as a hint when revealed. Judge every cloze as FREE PRODUCTION from the visible sentence alone. Never reason that "if this is multiple-choice, the distractors pin the answer" — they do not, for the learner who simply types. Well-chosen distractors are not a reason to raise \`qualityScore\` or to withhold an \`ambiguous\` flag the open blank has earned.
   - **Gloss consistency (cloze):** when the draft carries a \`glossEn\` (rendered above as **Meaning (shown to the learner)**), every \`acceptableAnswers\` entry must be true *under that gloss* — the learner reads the gloss, so it constrains the answer. An entry that changes the gloss's stated meaning — an antonym (\`cerca\` glossed "near" with \`lejos\` accepted), a different person (\`puedo\` glossed "I can't" with \`quiero\` accepted), a different hour, referent, or possessor — is a defect, not an alternant: set \`ambiguous = true\`. Preferred cure: **drop** the entries the gloss excludes, keeping the gloss tight. Only widen the gloss if it can stay genuinely vague about which answer applies — a gloss that itself names the alternatives (e.g. "I want/can walk to the park every day" for \`quiero\`/\`puedo\`) is a spoiler, not a fix, and is itself a defect. Exception: for a FORM point rather than a lexeme — the zero article before a profession, an adjective's declension ending — prefer dropping the gloss entirely over widening it; the alternates are legitimate and the gloss is simply over-specified.
   - **Neutral-gloss rule (cloze, lexical-choice points):** where the target language lexicalises or grammaticalises a distinction English collapses into ONE word — Spanish \`saber\`/\`poder\`, both glossed "can", where the blank tests which verb the meaning selects — the gloss MUST use the NEUTRAL English term and the L2 sentence MUST carry the contrast that forces the choice ("pero hoy no puede porque está cansada", "porque estudió mucho"). A gloss that reaches for the English lexical distinction ("know how to" for \`saber\`) or names the trigger ("(a learned skill)") is \`contextSpoilsAnswer = true\`. A neutral gloss is NOT \`ambiguous\` when the L2 sentence forces the reading — set \`ambiguous = true\` only when nothing in the L2 sentence disambiguates.
