...
// contract-split rule.
//
// 2026-07-18 (generate@2026-07-18): translation acceptableAnswers. The
// translation tool + content gained an `acceptableAnswers` field, so the
// Ambiguous-blank rule no longer FORBIDS a source with two structurally
// different correct renderings — it now REQUIRES enumerating them (or forcing
// one structure via person choice). Targets the tr-a1-gore-bence collapse
// ("In my opinion" → Bence vs. Bana göre, previously unenumerable → 0 approved
...
// the `ambiguous` flag.
//
// 2026-08-08 (generate@2026-08-08): substitute-back rule. A `correctAnswer`
// that restates the stem word beside the blank makes the exercise unanswerable
// as printed ("prefiero ___ del escaparate." / "el del" → "prefiero el del del
// escaparate"); 38 such rows were live in the prod pool, approved at 0.85-0.9
// because neither the structural parser nor the validator ever substituted the
// answer back. The rule also covers zero-form targets (zero article), whose
// only sound shape is to blank the whole slot including the noun. Enforcement
// is deterministic, not prompt-only: `checkClozeOverlap` (packages/ai) via
// `applyDeterministicChecks` rejects a multi-word duplication and flags a
// single-word one, so this rule only saves the wasted retry slot. NOT mirrored
// in validation-prompts.ts — the deterministic gate makes a validator rule
// redundant (contrast the generate↔validate contract-split cases above, where
// the validator had to learn a new accepted SHAPE).
//
// 2026-08-11 (generate@2026-08-11): polarity determinacy on comparative/
// superlative blanks. The superlative FRAME (`de todos`, `de toda la ciudad`,
// `del mundo`) is symmetric — it licenses `más` and `menos` identically — so a
// blank on the polarity slot whose stem carries nothing but the frame is
// undetermined, and a lone `correctAnswer` is a false-negative trap. Found in
// the live pool: of the 14 polarity-slot rows in es-b1-superlatives-comparisons
// (50 approved cloze rows, ZERO with `acceptableAnswers`), only 2 carried an
// evaluative anchor; the trap runs both ways, since one row answers `menos`
// with `más` equally valid. Since `showOptions` defaults to false in the drill
// UI, the learner types free text, so the distractor list does not disambiguate.
// The cure is an in-stem evaluative anchor, NEVER enumeration (`más`/`menos`
// are antonyms, not alternants — same logic as tense-determinacy and
// Form-contrast). Two corollaries ride along: an adjective-swallowing blank
// must be entailed by the stem, and the explicit-head-noun superlative order is
// `el/la + NOUN + más + ADJ` (the pool had `la más institución importante`).
// Template edit → Langfuse push per env; mirrored in validation-prompts.ts
// (validate@2026-08-11) per the generate↔validate contract-split rule.
...
// Bumped 2026-08-17 — vocab_recall gains a DISTINCT-referent rule: the existing
// near-synonym escape hatch (enumerate in `acceptableAnswers`) is scoped to words
// with the SAME referent, and explicitly does NOT license a definition that
// straddles two different things with the loser listed as an acceptable answer.
// Covers the three shapes the pool actually produced — superordinate/subordinate
// (Tisch/Schreibtisch), adjacent concepts (Wiese/Ebene, terminal/estación), and
// different word class or scale point (Nachhaltigkeit/nachhaltig, heiß/warm) —
// plus a hint/definition direction check. vocab_recall was carrying 26 of 61
// `ambiguous` flags on the 2026-08-17 run. Mirrored by validate@2026-08-17a; the
// two must land together or the fix is nullified. Template edit → Langfuse push
// per env.
// Bumped 2026-08-18 — anti-bypass rule for translation sources on contrasts
// English leaves OPTIONAL. All 50 approved es-b1-reported-speech translations
// carried an already-backshifted English source ("She said that she was
// tired"), so a word-for-word rendering produced the reference and the Spanish
// shift was never applied — the pool measured as full while testing nothing.
// English optionally backshifts (Butt & Benjamin 17.8 cites "John said he
// is/was coming"), so the source shape is the AUTHOR's choice here, which is
// what makes this a generation rule. Deliberately NOT mirrored in
// validation-prompts.ts: `contextSpoilsAnswer` is a hard veto, and a validator
// cannot distinguish "the author could have written it the other way" from
// "English forces this shape" — English prepositions pre-encode Turkish case
// and English infinitives pre-encode Spanish subjunctive complements, so a
// veto on pre-encoding sources would reject a large share of every translation
// cell in every language. The last clause of the rule marks that boundary for
// the generator instead. Template edit → Langfuse push per env.
// Bumped 2026-09-21 (generate@2026-09-21): lexeme determinacy on blanks that
// swallow the verb. Found from a learner flag on an approved
// es-b1-imperative-negative-pronouns cloze — "Tienes razon, ese problema es
// importante. ___ ahora, espera hasta manana." / `No lo comentes`, where
// `no lo menciones` / `no lo discutas` / `no lo hagas` are equally defensible:
// the blank absorbs the whole predicate, so the item tests which verb the
// author imagined on top of the mood+proclisis it means to test. 29 of the 34
// approved negative rows of that point carried neither an in-stem anchor nor a
// parenthetical verb, while the affirmative half was clean throughout because
// the unguessable accented form (`Daselas`) made the generator supply
// `(dar / se / las)` unprompted. The two cures are an entailing stem or a bare
// infinitive in parentheses; enumeration is explicitly NOT one, because the
// rival lexemes form an OPEN class and any acceptableAnswers list is therefore
// incomplete by construction (the flagged row DID list three alternates and
// still failed the learner). Same anchor-not-enumerate logic as the tense- and
// polarity-determinacy rules above. Mirrored by validate@2026-09-21; the two
// must land together or the fix is nullified. Template edit -> Langfuse push
// per env.
// Bumped 2026-09-22 (generate@2026-09-22): clitic-doubling determinacy on
// blanks that swallow a clitic cluster. Found from a learner report on
// session dc7654df — two approved es-b1-imperative-negative-pronouns clozes
// sharing the frame "___ a nadie, por favor." with OPPOSITE keys
// (`No se lo digas` / `No la cuentes`) and no acceptableAnswers on either.
// Spanish dative doubling over a POSTVERBAL dative PP is optional, so both
// +-se forms are standard and each row silently penalises the one it did not
// key. A paired probe over the real evaluateAnswer showed the judge inventing
// a rule in whichever direction the key pointed: `se` "is REQUIRED" on one row
// and "doesn't belong" on its twin. Unlike lexeme determinacy above, the rival
// set here is CLOSED (exactly two members), so enumeration IS the cure and the
 ...
 */
export function renderSentenceConstructionSection(
  exerciseType: ExerciseType,
  language: string,
  cefrLevel: string,
...
- **Scenario variety is REQUIRED.** Honor the assigned "Topic domain" for this draft; build the situation from that domain. Vary the scenario across drafts — do NOT default to "free time / more free time" or generic travel wish-lists, and do not reuse the same model answer across exercises.
...
- **NP-shaped nominal targets (article-marking languages).** Where ${language} realizes case/number on the ARTICLE and/or an ADJECTIVE rather than on the noun itself (German declension), \`targetForm\` is the smallest natural phrase that shows the marking — article + adjective + noun for article-bearing points (\`Tisch\` + indefinite/accusative → "einen neuen Tisch"), adjective + noun for article-less strong declension ("kaltem Wasser"), article + noun where only they inflect (n-declension: "den Studenten"). Gender comes from the given noun — state it inside \`featureBundle\`/\`features\` (e.g. {term: "Maskulinum", gloss: "masculine"}) so the learner can derive the endings; \`instructions\` must name the frame (which article type, and the adjective to use, chosen A2-simple: neu, klein, gut, kalt…). The whole phrase is graded as one exact string.
...
- **The learner must produce the answer themselves.** Two failure modes are forbidden:
  - **Ambiguous blank.** For a cloze, the answer must be uniquely produced. Either (a) the surrounding sentence constrains the blank so only one specific lexeme/form plausibly fits — every other candidate is ruled out by something explicit in the sentence — OR (b) for grammar-shape clozes where many lexemes satisfy the rule, you populate \`acceptableAnswers\` with every lexeme that fits. Sentences like "Sınıfta sekiz ___ var" ("There are eight ___ in the classroom") are forbidden without \`acceptableAnswers\`, because chair, student, book, pencil, and many other nouns all satisfy the rule equally. Likewise "Evde yeni ___ var. Onlar çok güzel." is forbidden with \`correctAnswer: "perdeler"\` alone — curtains, books, lamps, flowers all satisfy "plural, positive descriptor" equally; the "Onlar çok güzel" follow-on signals plurality but not which lexeme. Either constrain the sentence ("Evde yeni ___ var. Onları yıkamayı unutma." — "don't forget to wash them" picks out "perdeler") or list every plausible lexeme in \`acceptableAnswers\`. For translation, the reference translation is the anchor and minor surface variants are accepted at evaluation time. If the source admits two or more STRUCTURALLY DIFFERENT correct renderings (a different construction, not a surface tweak — e.g. TR "In my opinion…" maps to both \`Bence …\` (pronoun+-CE adverb) and \`Bana göre …\` (dative+göre)), you MUST enumerate every such alternative in \`acceptableAnswers\` (do NOT repeat the reference there); shipping a lone reference silently penalises the equally-correct alternative and is the generator-side cause of the validator's \`ambiguous\` flag. The alternative: rewrite the source so exactly ONE structure is natural — often by choosing a person that forces a single form (3rd person "In his/their opinion…" → only \`Ona/Onlara göre\`, which has no -CE variant). For vocab_recall, the prompt/definition must pick out exactly one headword — or, when the target language has true near-synonyms that any natural definition admits equally, \`acceptableAnswers\` must enumerate every other defensible headword (see the **vocab_recall near-synonyms** rule below). Escape hatch (b) is for interchangeable fills only — when the grammar point itself CONTRASTS the competing forms, see the **Form-contrast clozes** rule below instead.
...
- **Substitute your answer back before submitting — no word may be printed twice.** Mentally replace the \`___\` in \`sentence\` with \`correctAnswer\` and read the result. It MUST be exactly the sentence you intended, with no word appearing twice in a row. A \`correctAnswer\` MUST NOT restate a word that is still visible in the stem next to the blank: "No me gusta este abrigo; prefiero ___ del escaparate." with \`correctAnswer: "el del"\` substitutes to "prefiero **el del del** escaparate" — broken; either blank the whole phrase ("prefiero ___ escaparate." → \`el del\`) or shorten the answer to what the visible stem admits ("prefiero ___ del escaparate." → \`el\`). Same defect: "Ich habe ___ Bruder." → \`einen Bruder\` ("einen Bruder Bruder"), "Die Kinder laufen ___ Park." → \`in den Park\`, "Wegen ___ Regens…" → \`des Regens\`. The rule holds when the target is a **zero** form too: for a zero-article/no-article point the blank cannot be empty, so blank the whole slot INCLUDING the noun — "Mein Vater ist ___." → \`Arzt\` — never "Mein Vater ist ___ Arzt." → \`Arzt\`. A word repeated elsewhere in the sentence is fine; only adjacent duplication caused by the substitution is the defect. This is checked deterministically after generation — a draft that duplicates a word is rejected outright, so a retry slot is wasted every time you ship one.
...
- **Context-cued tense/aspect clozes — generic instruction, context-forced tense.** For a cloze whose grammar point is *which tense or aspect to use* in a context that already cues it — the paradigm case is German \`seit\` + **present** tense for a situation ongoing since a past point (where English uses the present perfect: "has lived … for three years") — the \`instructions\` MUST be generic ("Fill in the blank with the correct form of the verb in parentheses") and MUST NOT name the target tense/aspect or state the rule's outcome. Naming the tense — "the correct **present-tense** form", or a reminder like "ongoing situations use seit + present tense" — spells out the very choice the blank tests and is the generator-side cause of the validator's \`contextSpoilsAnswer\` veto. Instead the sentence's time context MUST force exactly that tense as the only natural answer: \`seit\` + a duration phrase with an ongoing situation ("Mein Bruder ___ seit drei Jahren in München. (wohnen)" → \`wohnt\`) makes the present the sole idiomatic German form — the perfect would read as completed and contradict the ongoing reading — so dropping the tense name does NOT make the blank \`ambiguous\`. The parenthetical hint is the verb's citation (infinitive) form. This rule is for tense/aspect **choice** points only; it does NOT apply to points that test *forming* a named tense (e.g. Perfekt formation, Präteritum), where naming the tense is the task itself, not the answer being tested.
...
- **Optional-in-English contrasts — do NOT pre-resolve them in the translation source (anti-bypass).** For a translation whose grammar point is a choice the target language FORCES but English leaves OPTIONAL, the English source MUST be written in the shape that leaves the choice open, never the shape that already spells out the answer — otherwise a word-for-word rendering produces the reference translation and the item tests nothing. The paradigm case is **reported speech / sequence of tenses**: English backshifts optionally ("She said she **is** tired" and "She said she **was** tired" are both natural), while Spanish requires the shift after a past reporting verb. A source that arrives ALREADY backshifted — "She said that she was tired" → \`Dijo que estaba cansada\` — hands the learner the past-tense complement, so the answer falls out of a literal rendering and the shift is never applied. Write the complement in the tense the ORIGINAL speaker used: "She said she **is** tired" → \`Dijo que estaba cansada\`, "He told me he **doesn't have** time" → \`Me dijo que no tenía tiempo\`. Symmetrically, for a construction that must NOT shift, keep the English reporting verb in the present zone so the retained present is a decision and not a copy: "He**'s said** he**'s coming**" → \`Ha dicho que viene\`. This rule is ONLY for contrasts English genuinely leaves optional — where English FORCES a shape that happens to match the target ("I'm going **to** school" pre-encoding a dative, an English infinitive after a verb of influence), the source is not bypassable and needs no rewriting; do not distort natural English to hide a structure English has no choice about.
- **Do not leak the answer in the visible text (anti-leak).** Apart from the parenthetical citation hint — which is the word to *inflect*, not the answer form — nothing in the visible \`sentence\`, \`glossEn\`, or \`instructions\` may let the learner write the blank without engaging the grammar point. Concretely forbidden: the target's inflected form (or the bare lexeme, when the task is lexical) appearing elsewhere in the sentence; an L1 gloss or near-synonym that names the exact target word; a cue phrase that makes the fill mechanical. Negative example — "Bu ___ çok eski. Bu kitabı dün aldım. (kitap)" leaks "kitabı" in the next clause, and a \`glossEn\` like "I drink the coffee (kahveyi)" that spells out the inflected form is forbidden. This is the generator-side guard for the validator's \`contextSpoilsAnswer\` veto, which remains in force. **Neutral gloss for lexical-choice points (cloze).** Where {{language}} lexicalises or grammaticalises a distinction English collapses into ONE word, do NOT reach for the English distinction in \`glossEn\` — gloss with the NEUTRAL English term and make the L2 sentence carry the contrast that forces the choice. Spanish saber/poder is the canonical case: English "can" covers both, so gloss "can" and force the reading in Spanish ("Mi abuela ___ contar historias muy bien, pero hoy no puede porque está cansada" → \`sabe\`; "Mi hermana ___ hablar italiano muy bien porque estudió mucho" → \`sabe\`). A gloss saying "know how to" names the answer, and a parenthetical naming the trigger ("(a learned skill)") is the same defect. If the L2 sentence cannot be made to force the choice, OMIT the gloss rather than leak it.
...
- **Tense determinacy on finite-verb blanks.** When the blank is a FINITE VERB and tense/aspect is NOT the feature this grammar point drills (e.g. causative/permissive dejar/permitir/hacer + infinitive, ser/estar, por/para — the verb's tense is incidental, not the target contrast), the visible sentence MUST admit exactly ONE tense. Either (a) put a temporal anchor IN the stem that forces it — a preterite/imperfect/pluperfect verb elsewhere in the sentence (\`Cuando llegué tarde…\`, \`El viento era tan fuerte…\`, \`habíamos logrado…\`) or an explicit past adverbial (\`ayer\`, \`anoche\`, \`esa noche\`, \`la semana pasada\`) — OR (b) target the PRESENT, the default reading of a generic statement whose stem carries NEITHER a past anchor NOR a habitual/iterative adverbial (the latter re-admits the imperfect — see next). A non-present \`correctAnswer\` in an anchorless stem is FORBIDDEN: \`El portero no ___ entrar al mensajero sin identificación.\` → \`dejó\` is a false-negative trap, because \`deja\` (a standing rule) is equally correct given only the visible text. Habitual/iterative cues — \`siempre\`, \`cada vez que\`, \`todos los días\`, \`por las noches\`, \`normalmente\`, \`a menudo\` — FORBID the preterite (a single completed event cannot be habitual), but they do NOT force the present: they license the present-habitual AND the IMPERFECT (Spanish's past-habitual/iterative tense) equally, so on their own they leave the blank present↔imperfect ambiguous. \`El ruido de la calle no me ___ dormir por las noches.\` admits both \`deja\` (present) and \`dejaba\` (past habitual); \`El entrenador nos ___ correr diez kilómetros todos los días.\` admits both \`hace\` and \`hacía\` — never \`hizo\`. To make such a blank determinate you MUST add an anchor that fixes the present/past axis, not merely the habitual reading: a PRESENT anchor (\`ahora\`, \`hoy en día\`, \`estos días\`, \`últimamente\`, or another present-tense verb in the stem) forces the present; a past anchor forces the imperfect. A bare habitual adverbial is NOT a present anchor, so it does NOT license shipping a lone present \`correctAnswer\`. This is same-lexeme tense ambiguity, reinforcing the **Ambiguous blank** and **One correct fill** rules above; the cure is anchor-or-present, NEVER enumerating tense variants in \`acceptableAnswers\` (listing \`deja, dejó\` teaches they are interchangeable, which they are not — same logic as the Form-contrast rule below).
- **Polarity determinacy on comparative/superlative blanks.** When the blank sits on the POLARITY slot of a scalar comparison — ES \`más\`/\`menos\` (with or without the article: \`más\`, \`el más\`, \`la menos\`), DE \`am meisten\`/\`am wenigsten\`, TR \`en\` vs. \`en az\` — the visible sentence MUST make exactly ONE pole natural. The superlative FRAME is SYMMETRIC: \`… de todos\`, \`de toda la ciudad\`, \`del mundo\`, \`de todos los que hice este año\` license \`más\` and \`menos\` equally well, so a stem whose only content is the frame leaves the blank undetermined and a lone \`correctAnswer\` is a false-negative trap. FORBIDDEN: \`El examen de matemáticas fue ___ difícil de todos los que hice este año.\` → \`el más\`, because \`el menos difícil\` is equally correct on the visible text; likewise \`Este remedio es el ___ efectivo de todos los que he probado.\` and \`Adoptar un perro es la decisión ___ importante de mi vida.\`. The trap runs BOTH ways — a \`menos\` answer in an unanchored stem (\`Esta película es la ___ aburrida de todas las que hemos visto este año.\`) is equally forbidden, since \`más aburrida\` is just as defensible. The cure is an in-stem EVALUATIVE anchor — a consequence, reaction, or fact about the situation that fits only one pole: \`Este parque es el ___ bonito de toda la ciudad, y por eso lo amamos tanto.\` (the affection forces \`más\`), \`Este mercado es ___ barato de todo el barrio, así que venimos aquí cada semana.\` (the weekly return forces \`más\`), \`Esta película es la ___ aburrida de todas las que hemos visto este año — no despegamos los ojos de la pantalla.\` (not looking away forces \`menos\`). The anchor MUST be a fact about the situation, never a restatement of the pole itself (do not write "…y es carísimo" to force \`más caro\` — that leaks the answer; the anti-leak rule stays in force). Enumeration is NEVER the cure here: \`más\` and \`menos\` are ANTONYMS, not interchangeable alternants, and listing both in \`acceptableAnswers\` teaches the learner they mean the same thing — same logic as the tense-determinacy rule above and the Form-contrast rule below. Two corollaries: (a) if the blank swallows the ADJECTIVE as well (\`Este hotel es el ___ de toda la ciudad, pero también es el más cómodo.\` → \`más caro\`), the stem MUST entail that exact adjective — otherwise \`más grande\`, \`más ruidoso\`, and \`más lujoso\` are all defensible and the item tests mind-reading rather than the grammar point; (b) when the superlative has an explicit head noun, the standard Spanish order is \`el/la + NOUN + más/menos + ADJ\` (\`la institución más importante del barrio\`), so a blank positioned to yield \`la más institución importante\` is malformed — substitute the answer back and check the sentence reads correctly before submitting.
- **Lexeme determinacy on blanks that swallow the verb.** When the blank contains the verb's LEXICAL STEM — not merely an inflection of a verb already visible in the stem — and this grammar point drills a FORM (mood, clitic placement, agreement, periphrasis) rather than which verb the meaning selects, the visible sentence MUST make exactly ONE lexeme natural, or the verb MUST be handed to the learner. A blank that absorbs the whole predicate silently adds a second, untested task: guessing which verb the author had in mind. FORBIDDEN: \`Tienes razón, ese problema es importante. ___ ahora, espera hasta mañana.\` → \`No lo comentes\`, because \`no lo menciones\`, \`no lo discutas\`, \`no lo toques\`, \`no lo decidas\`, and \`no lo hagas\` are all equally defensible on the visible text; likewise \`Tienes los apuntes, pero no ___ a nadie.\` → \`se los prestes\` (\`se los des\` fits identically) and \`El libro no es tuyo — ___ de la mesa.\` → \`no lo robes\` (\`no lo quites\` / \`no lo cojas\` / \`no lo saques\`). Two cures, in order of preference: (a) make the stem ENTAIL the verb — name the object, consequence, or reaction that only one verb fits (\`Esta chaqueta es muy cara. ___ todavía; espera a las rebajas.\` → \`No la compres\`, where the sales force \`comprar\`; \`El pastel está muy rico. ___ todo ahora — guarda un poco para mañana.\` → \`No te comas\`); or (b) give the verb's CITATION (infinitive) form in parentheses next to the sentence (\`Tienes que hablar con tu jefe hoy — ___ mañana. (irse)\` → \`no te vayas\`). The parenthetical MUST be the bare infinitive and MUST NOT carry the clitics or the inflection the item tests — \`(enviar)\`, never \`(enviarlo)\` (which hands over the pronoun whose PLACEMENT is the point) and never \`(no lo envíes)\` (which spoils the blank outright); when the point is clitic placement, the referent the clitic stands for must still be recoverable from the sentence. **Enumeration is NOT a cure here.** Unlike a closed pair of near-synonyms, the verbs that fit an unanchored predicate frame are an OPEN class — listing \`comentes, menciones, digas, cuentes\` in \`acceptableAnswers\` still fails the learner who writes \`discutas\` or \`hagas\`, and every omission is a false-negative trap. This is the same logic as the tense-determinacy and polarity-determinacy rules above: anchor the blank, do not enumerate around it. The rule does NOT apply when the lexical choice IS the grammar point (ser/estar, saber/poder, por/para, verbs of change) — there the contrast is the task, and the **Form-contrast** rule below governs instead.
- **Clitic-doubling determinacy on blanks that swallow a clitic cluster.** When the blank contains the CLITIC(S) and the visible stem also carries an OVERT POSTVERBAL DATIVE phrase — ES \`a nadie\`, \`a alguien\`, \`a tu hermano\`, \`a los niños\`, \`a María\` — Spanish leaves the dative clitic OPTIONAL: \`No se lo digas a nadie\` and \`No lo digas a nadie\` are both standard, as are \`no se lo des a tu hermano\` and \`no lo des a tu hermano\`. A lone \`correctAnswer\` picking one of them is therefore a false-negative trap in whichever direction it points, and the trap is symmetric: keying the doubled form penalises the learner who omits \`se\`, keying the undoubled form penalises the learner who supplies it. FORBIDDEN: \`Esta historia de fantasía es un secreto. ___ a nadie, por favor.\` → \`No la cuentes\` alone; \`La verdad ya salió a la luz — ___ a nadie, por favor.\` → \`No se lo digas\` alone; \`Este pastel está hecho con mucho amor — ___ a los niños antes de la cena.\` → \`no se lo des\` alone. **Enumeration IS the cure here** — and this is the one place the determinacy family diverges. The rival set is CLOSED: exactly TWO members, the cluster with and without the dative clitic. So \`acceptableAnswers\` MUST list the counterpart of whichever form you key (\`correctAnswer: "No la cuentes"\` → \`acceptableAnswers: ["No se la cuentes"]\`), and listing it teaches nothing false, because the two forms genuinely ARE interchangeable here — unlike \`deja\`/\`dejó\`, \`más\`/\`menos\`, or an open lexeme class, where enumeration is forbidden precisely because the members are not equivalent. The alternative cure is to remove the ambiguity at the source: drop the overt dative PP so the recipient is recoverable only from context and the clitic becomes obligatory (\`Tu hermana quiere saber la noticia — ___ todavía.\` → \`No se la digas\`), or give the cluster in parentheses as the affirmative rows already do (\`___ a tu hermano ahora mismo, por favor. (dar / se / las)\` → \`Dáselas\`). Do NOT apply this rule in two cases: (a) when the dative phrase is PREVERBAL / left-dislocated (\`A tu hermano ___ cuidan muy bien en ese hospital.\`), where doubling is OBLIGATORY and the blank is already determinate; and (b) when the point DRILLS the doubling itself (es-a2-indirect-object-pronouns-se), where the clitic is the task and the overt PP is the intended cue. The rule is about DATIVE doubling only — accusative doubling is not free in Spanish and an \`a + person\` DIRECT object (\`No llames a tu madre\`) does not license an extra clitic.
...
- **vocab_recall DISTINCT referents — \`acceptableAnswers\` is NOT a cure; tighten the definition.** The near-synonym rule above applies ONLY to words with the SAME referent (\`istasyon\`/\`gar\`, \`coche\`/\`auto\`) — two labels for one thing, where either answer is genuinely right. It does NOT license writing a definition that straddles two DIFFERENT things and then listing the loser in \`acceptableAnswers\`. Three shapes to avoid, all real defects found in the pool:
  (a) **Superordinate vs. subordinate.** DE \`der Tisch\` (any table) vs \`der Schreibtisch\` (a desk). A definition reading "flat surface and legs, often found in a kitchen or office, where you eat, write, or work" mixes cues belonging to BOTH and picks out neither — listing \`der Tisch\` alongside \`der Schreibtisch\` does not repair it. Fix by DELETING the neighbour's cues, not by adding answers: for \`der Schreibtisch\` keep office/writing/computer and drop kitchen/eat entirely.
  (b) **Adjacent-but-different concepts.** DE \`die Wiese\` (meadow) vs \`die Ebene\` (plain); ES \`la terminal\` vs \`la estación\`; DE \`die Rezension\` (review of a book/film) glossed with restaurant cues, which elicits \`die Kritik\`. Name the feature that is TRUE of the target and FALSE of the neighbour.
  (c) **Different word class or scale point.** Never list an adjective as an acceptable answer for a noun target (\`Nachhaltigkeit\` vs \`nachhaltig\`), nor a different degree on the same scale (\`heiß\` vs \`warm\` — "very high temperature, the opposite of cold" is \`heiß\`, and \`warm\` is a different claim, not an alternant).
  Self-check before emitting: name the single closest other word in this umbrella, then confirm your definition contains at least one cue that is TRUE of \`expectedWord\` and FALSE of that neighbour, and NO cue that is true only of the neighbour. If you cannot write such a cue at this CEFR level, the two really are synonyms — then, and only then, enumerate. \`hints\` must pull the SAME direction as the definition; a prompt leaning one way with a hint leaning the other is itself the defect.
- **vocab_recall kinship terms — the definition MUST pin the same relation the answer names.** Some languages lexicalize a kin relation that English collapses into one word — most sharply by SIDE of the family. Turkish is the canonical case: an uncle is \`amca\` (father's brother) vs \`dayı\` (mother's brother); an aunt is \`hala\` (father's sister) vs \`teyze\` (mother's sister); a grandmother is \`babaanne\` (father's mother) vs \`anneanne\` (mother's mother). For such a side-specific \`expectedWord\`, a side-NEUTRAL English gloss ("your father's OR mother's brother", "your parent's sister") is genuinely true of BOTH terms and is therefore \`ambiguous\` — it is NOT a valid clue. You MUST either (a) name the side in the definition so exactly one term fits ("your MOTHER's brother" → \`dayı\`; "your FATHER's sister" → \`hala\`), or (b) if you deliberately want the side-neutral concept, set \`expectedWord\` to the most common term and enumerate the other in \`acceptableAnswers\` (both \`amca\` and \`dayı\` answer "your parent's brother"). A generic umbrella term that IS side-neutral in the target language (TR \`büyükanne\`/\`dede\` grandparent, \`kuzen\` cousin, \`akraba\` relative) may keep a side-neutral gloss — the side-pinning requirement applies only when the language forces the distinction. Separately and absolutely: the definition MUST describe the SAME relation the \`expectedWord\` denotes — never gloss \`dayı\` as "the son or daughter of your mother's brother" (that is a cousin, \`kuzen\`) or \`hala\` as "your mother's sister" (that is \`teyze\`). A clue that names a different relative than the answer is simply wrong, not merely ambiguous.
