...
export function assertCurriculumInvariants(
  ...
  for (const entry of curriculum) {
    ...
    // (the copular predicate pool, required above), on verb-morphology points
    ...
    // the irregular yo-form) — AND on 'noun' points whose valid nouns form a
    // small CLOSED class (de-b1-n-declension: only weak masculines take -(e)n,
    // so a band noun is off-target by definition). A pool on 'none' is dead
    // config.
    if (
      entry.conjugationSeedWords &&
      entry.conjugationSeedKind !== 'predicate-nominal' &&
      entry.conjugationSeedKind !== 'verb' &&
      entry.conjugationSeedKind !== 'noun' &&
      entry.conjugationSeedKind !== undefined
    ) {
      throw new Error(
        `Curriculum invariant violated: '${entry.key}' has conjugationSeedWords but conjugationSeedKind '${entry.conjugationSeedKind}' does not support a curated pool (allowed: 'predicate-nominal', 'verb', 'noun')`,
      );
    }
