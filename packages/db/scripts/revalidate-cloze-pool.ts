...
// `--ids-file` handling lives in ./lib/ids-file.ts — `demote:pool` needs the
// same parse and path-resolution rules, and they are a contract an operator
// learns once. Re-exported here so this module's existing importers and tests
// keep their entry point.
export { idsFileCandidates, parseIdsFile } from './lib/ids-file';
