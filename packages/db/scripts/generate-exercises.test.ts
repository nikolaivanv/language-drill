...
describe('parseGenerateArgs', () => {
  ...
  // Phase 4 — --queue flag
  it('accepts --queue and sets queue=true', () => {
    const args = parseGenerateArgs(['--lang', 'es', '--level', 'B1', '--queue']);
    expect(args.queue).toBe(true);
  });

  it('rejects --queue with --concurrency >1 naming the constraint', () => {
    expect(() =>
      parseGenerateArgs([
        '--lang',
        'es',
    ...
    ).toThrow(/--queue is incompatible with --concurrency >1/);
  });

  it('accepts --queue --dry-run together', () => {
    const stderrSpy = vi
      .spyOn(process.stderr, 'write')
      .mockImplementation(() => true);
    try {
