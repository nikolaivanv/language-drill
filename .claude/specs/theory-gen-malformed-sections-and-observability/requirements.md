...
7. WHEN unit tests run THEN `theory.ts` parser tests SHALL cover: (a) a native `sections` array passes unchanged, (b) a valid stringified `sections` is decoded, (c) a malformed stringified `sections` with unescaped inner quotes (the captured 2026-06-01 payload shape) is recovered, and (d) an unrecoverable garbage string still throws a clear error. `theory-generate.test.ts` SHALL cover the regenerate-on-malformed retry loop (success on a later attempt; exhaustion after max retries).
