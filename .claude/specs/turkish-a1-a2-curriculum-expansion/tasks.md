...
- [ ] 1. tr.ts — header/version + A1 verb system (5 entries)
...
- [ ] 2. tr.ts — A1 verbless predicates (3 entries)
  - File: packages/db/src/curriculum/tr.ts
...
- [ ] 3. tr.ts — A1 noun cases (3 relocated entries)
  - File: packages/db/src/curriculum/tr.ts
...
- [ ] 4. tr.ts — A1 reference words, numbers, possessives (4 entries)
  - File: packages/db/src/curriculum/tr.ts
...
- [ ] 5. tr.ts — A1 postpositions/particles + comparison (7 entries)
  - File: packages/db/src/curriculum/tr.ts
...
- [ ] 6. tr.ts — A2 tenses + modality (3 entries)
  - File: packages/db/src/curriculum/tr.ts
...
- [ ] 7. tr.ts — A2 converbs, nominalization, relative (4 entries)
  - File: packages/db/src/curriculum/tr.ts
...
- [ ] 8. tr.ts — A2 connectives, emphasis, purpose, reported speech (7 entries)
  - File: packages/db/src/curriculum/tr.ts
...
- [ ] 9. tr.ts — prune/narrow disabled B1/B2 block
  - File: packages/db/src/curriculum/tr.ts
...
- [ ] 11. curriculum.test.ts — update TR counts + run the invariant gate
...
- [ ] 12. seed-exercises.ts + deterministic-uuid.test.ts — re-key/re-level TR seeds
...
- [ ] 13. migration 0014 — delete orphaned TR A2 rows
  - Files: packages/db/migrations/0014_tr_a1_realign_cleanup.sql, packages/db/migrations/meta/_journal.json
  - Add the SQL exactly as in design.md (4 DELETEs separated by `--> statement-breakpoint`: exercise_tags by the 5 skill_topic UUIDs → exercise_tags by orphaned exercise ids → exercises by the 5 old keys (language='TR') → skill_topics by the 5 UUIDs). Append a journal entry `{ idx: 14, version: "7", when: <epoch-ms>, tag: "0014_tr_a1_realign_cleanup", breakpoints: true }`.
  ...
  - _Leverage: design.md Migration 0014 SQL + the 5 precomputed UUIDs; existing migrations 0001–0013 for format_
  - _Requirements: 18_

- [ ] 14. Repo-root build gate
