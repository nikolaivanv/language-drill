...
- [ ] 4. Update evaluation tests for the new model + retry options
...
- [ ] 5. Implement `extractCompletedFields` partial-JSON parser
...
- [ ] 6. Add byte-boundary contract tests for `extractCompletedFields`
...
- [ ] 7. Implement `streamSpan` generator + `ReadSpanStreamMaxTokensError`
...
- [ ] 8.1. Set `maxRetries: 1` on the skim `streamAnnotation` call
...
- [ ] 9. Export the new streaming symbols from the `packages/ai` barrel
...
- [ ] 10. Extend the SSE writer with a `field` event type
...
- [ ] 11. Add the deep-span request schema
...
- [ ] 12. Extract shared handler gates + flush into reusable helpers
...
- [ ] 13a. Deep-flow pre-model gates: validation, span type, cache hit, rate-limit, profile
...
- [ ] 13b. Deep-flow model stream + side effects: trace, streamSpan loop, write-back, meter, terminal
...
- [ ] 14. Wire path dispatch into the handler
  - File: `infra/lambda/src/annotate-stream/handler.ts`
...
- [ ] 15. Route the deep path in the local dev server
...
- [ ] 16. Add handler/deep-flow integration tests
...
- [ ] 17. Remove the Hono `/read/annotate-span` route
...
- [ ] 18. Add deep-stream SSE event schemas
...
- [ ] 19. Implement `useReadAnnotateSpanStream` hook
...
- [ ] 20. Add `useReadAnnotateSpanStream` tests + barrel export
...
- [ ] 21. Add a progressive `DEEP_CARD_FIELD` action to the read-page reducer
...
- [ ] 22. Swap the read page to the streaming span hook and retire the old hook
...
- [ ] 23. Update read page component tests
...
- [ ] 24. Update the deep-card E2E mock to SSE
...
- [ ] 25. Update the Function URL CFN output description
...
- [ ] 26. Write the Groq/Cerebras fast-inference exploration doc
...
- [ ] 28. Eval-harness gate for the Haiku swap (validation step, may require maintainer creds)
