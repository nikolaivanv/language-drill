...
| `pnpm revalidate:cloze` | One-off CLI to re-score stored clozes through the current validator and demote failures. Dry-run by default; pass `--apply` to write. **A dry-run costs the same as an apply** — only writes are gated, so budget one pass, not two (the whole cloze pool is ~13k rows at ~$0.008/row). Narrow the sweep with `--grammar-point <key>` (repeatable, validated against the curriculum) or `--ids-file <path>` (one exercise id per line, `#` comments allowed — lets a read-only SQL pre-filter pick the rows so no token is spent outside the defect class). `--deterministic-only` runs **only** the pure checkers (answer/stem overlap any language, Turkish vowel harmony) — zero LLM calls, no `ANTHROPIC_API_KEY` needed, and it never overwrites a row's `quality_score` or drops its existing `flagged_reasons`. Also supports `--language`, `--cefr`, `--limit`, `--concurrency`, `--max-cost-usd`. |
| `pnpm revalidate:promote` | The **promote-only** mirror of `revalidate:cloze`: re-scores already-stored `flagged` rows through the current validator and promotes the ones that now auto-approve to `manual-approved` (clearing `flagged_reasons`). It never lowers a status, and `auto-approved` / `manual-approved` / `rejected` rows are skipped entirely. Use it **only after fixing a specific validator over-flag bug** — the parser enforces that rather than trusting a comment: `--type <exercise-type>` is **required** (no pool-wide sweep by omission) and `--grammar-point <key>` (repeatable, curriculum-validated) is **required for every type except `sentence_construction`**, the one type whose whole-pool sweep is the historical PR #606 case. A dry-run costs the same as an apply (only the write is gated). Dry-run by default; `--apply` writes. Also supports `--language`, `--cefr`, `--limit`, `--concurrency`, `--max-cost-usd`. |
| `pnpm demote:pool` | One-off CLI to demote a single cell's approved exercises back out of the pool. Requires `--reason quality\|learner-flag\|duplicate\|pool-hygiene` — `quality` and `learner-flag` also revoke learners' credit for past attempts on those rows (and print a reminder to re-run `backfill:mastery`), while `duplicate` and `pool-hygiene` leave scores untouched. Supports `--limit <n>` to cap demotion at the oldest `n` matching rows (by `created_at`), leaving the rest in the pool. `--ids-file <path>` names the rows outright (one exercise id per line, `#` comments allowed — same format as `revalidate:cloze`), for a subset the other filters cannot express: `--content-ilike` reads `content_json`, so a selection driven by `coverage_tags` (`case`/`number`, a separate column) is otherwise unreachable. Ids narrow **on top of** the cell filters, so a stale worklist cannot reach another cell; the flag is mutually exclusive with `--limit` and `--content-ilike`; and any id that does not match an approved row in the named cell **aborts the run** naming the offenders, rather than silently demoting fewer rows than the file lists. Dry-run by default; pass `--apply` to write. |
| `pnpm backfill:demotion-reason` | One-off classifier filling `exercises.demotion_reason` for rows demoted before the column existed: validator/revalidator flagged reasons → `quality`, upheld learner flags → `learner-flag`, everything else → `pool-hygiene`. Dry-run by default; pass `--apply` to write. Idempotent. |
| `pnpm backfill:coverage-tags` | Replays the validator over approved rows and writes `exercises.coverage_tags`. **Costs API calls even in dry-run** (only the write is gated by `--apply`), like `revalidate:cloze`. Default selection is `coverage_tags IS NULL`; `--include-partial` **also** takes rows whose tags exist but lack an axis the point's `coverageSpec` declares — the case a spec axis added *after* a cell was generated creates, where every row is tagged, none carries the new key, and `audit:collapse` reports the axis as 0-realized over sound content. `IS NULL` alone never reaches those rows (the same blind spot as the `seedWord IS NULL` guard that made the #631 variant backfill a no-op on 96% of its rows), so **scope `--include-partial` with `--grammar-point`/`--type`** or it sweeps most of the pool. Writes **merge** over existing tags, never replacing, so re-tagging can only add axes. Dry-run by default; `--apply` writes. Supports `--language`, `--cefr`, `--grammar-point`, `--type`, `--include-partial`, `--limit` (rows), `--concurrency`, `--max-cost-usd`. |
| `pnpm backfill:variant-seeds` | One-off CLI labelling the approved cloze / translation / **sentence-construction** pool of the `constructionVariants` points with the variant id each row actually realizes (SC joined 2026-08-20: `seedKindFor` has routed it to variant seeding since #652, but the legacy SC pool was unreachable, leaving those cells at zero measured coverage — which `pickVariantSeeds` reads as "nothing is covered" and answers by spreading drafts evenly) — the prerequisite for the PR #631 repass. Batches ~20 rows per Claude call within a cell (the point's variant list is the cached system block) and writes `content_json.seedWord` **keyed on the row's primary key**, never on a content pattern. A `null` classification never writes, and only `high` confidence writes unless `--min-confidence medium`. Dry-run by default; `--apply` additionally requires `--snapshot <neon-branch-id>` or an explicit `--no-snapshot`. Every run writes a replayable rollback artifact to `./backfill-runs/<name>.json`; `--revert <artifact> --apply` restores it (only the rows an applied run actually wrote). **Name every applied run** with `--name <run>` — the artifact is the only fine-grained record of its rows' original `seedWord`, so a second default-named run would clobber it; overwriting an applied artifact is refused unless `--force`. Supports `--language`, `--cefr`, `--grammar-point` (validated against the curriculum), `--max-cells N` (caps **N cells**, not rows — `--limit` is an alias, but note it means rows in `revalidate:cloze` / `backfill:coverage-tags`), `--batch-size`, `--concurrency`, `--max-cost-usd`, `--name`, `--force`. Verify with `pnpm audit:collapse --dry-run` afterwards. |
...
| `pnpm audit:gloss` | **Read-only, never writes to the pool.** Sweeps approved cloze rows carrying a learner-visible `glossEn` for spoilage — a gloss that states a rule's TRIGGER or OUTCOME (e.g. "(a current condition)" handing over `está`) instead of only conveying meaning the learner must still convert into a form. Two signals keep the ~1,568-row sweep affordable: a per-point triage call (does English even mark the distinction this point's blanks test — "The coffee is on the table" cannot leak ser/estar) that excludes a whole point when it doesn't, and a per-row judge over every row of a surviving point **plus every row carrying a parenthetical regardless of its point's verdict**, since a parenthetical can leak inside an otherwise-safe point. A failed triage call never excludes a point's rows — a missing verdict is treated as "judge them." Writes JSON + markdown to `./audit-runs/`, with a `loadBearing` row section called out separately (the offending span is the only thing forcing the reading, so trimming it would make the blank ambiguous — authoring work, not a data edit). Remedy (editing or dropping a gloss) is a separate reviewed step. Supports `--language`, `--cefr`, `--grammar-point`, `--limit`, `--max-cost-usd`, `--dry-run` (no API calls; prints a rough would-judge estimate, writes no report). A spotlight, not a gate. |
| `pnpm audit:constructions` | **Read-only, never writes to the pool.** Sweeps the approved cloze/translation pool for points whose description claims several constructions but whose pool realizes one — the PR #664 defect class, invisible to `audit:collapse` (two of its three signals read declared mechanisms a spec-less point lacks by definition; the other two are lexical, so 45 rows of one construction over 45 different nouns look diverse). Four stages: enumerate the constructions a point claims and label each must-represent vs. illustrative; classify a deterministic, seeded per-cell sample against them; flag any must-represent construction at 0 or ≤5% of classified rows; and, for confirmed findings, print a paste-ready `constructionVariants` / `coverageSpec` snippet for human review. A cell whose sampled rows are >33% unresolved reports as `enumeration-suspect` and raises **no** finding — a high unresolved rate means the construction list was wrong, not that the pool collapsed. **`--dry-run` makes no API calls and costs nothing** (unlike `revalidate:cloze` / `backfill:coverage-tags`, where a dry-run costs the same as an apply) and prints a rough dollar estimate; `--max-cost-usd` **defaults to $2**, well under a full sweep's ~$19, so an unflagged run truncates — check the `--dry-run` estimate first. `--concurrency` parallelizes classification batches **within** one cell; cells and points are still processed serially. `--max-points` caps points selected from the curriculum **before** filtering to those with an approved cell, so fewer than N may actually be examined. `--out <name>` names the run (`audit-runs/<name>.json`/`.md`), not a literal path. `--enumeration-model` overrides only the enumeration-stage model — the cost cap and every reported cost figure stay Sonnet-priced regardless, so an Opus enumeration run can silently overspend the cap; the CLI prints a warning when the flag is set. Writes JSON + markdown to `./audit-runs/`. Supports `--language`, `--cefr`, `--grammar-point`, `--type`, `--max-points` (**not** `--limit`), `--min-rows`, `--sample-per-cell`, `--seed`, `--max-cost-usd`, `--concurrency`, `--enumeration-model`, `--out`, `--check-fixture`. A spotlight, not a gate. |
...
| `pnpm --filter @language-drill/ai propose:book-coverage` | LLM-assisted book-coverage ledger authoring (see `docs/superpowers/specs/2026-07-15-book-coverage-ledger-design.md`). `--book-dir <mirror> --language es\|de\|tr --emit-toc` generates the vendored TOC snapshot from the reference-grammar mirror's `index.json`; `--chapter <n>` proposes one claim/exclude/GAP decision per section of that chapter, grounded in the chapter text + the language's grammar points, and prints a paste-ready ledger fragment for human review + commit into `packages/db/src/curriculum/book-coverage/`. Ledgers registered there are enforced by `book-coverage.test.ts` (every book section accounted for; no dangling point keys). Ledger edits do **not** bump `CURRICULUM_VERSION_*`. In-repo prompt (not Langfuse). |
