 ...
 *   4. Saturated-dedup → `skip-saturated-dedup` (R6.2; beats low-yield)
 *   5. Low-yield → `skip-low-yield` (R1.4)
 *   6. Otherwise → `enqueue` with `need = TARGET - approvedInPool`
 */
...
export function decideEnqueue(
  ...
  // 2. Target-reached. R1.3 — the cell already has enough approved exercises;
  //    no further enqueueing needed.
  if (approvedInPool >= TARGET_PER_CELL) {
    return { kind: 'skip-target-reached' };
  }

  const need = TARGET_PER_CELL - approvedInPool;
  ...
  // 5. Saturated-dedup detection (R6.1 + R6.2). Takes precedence over
  ...
  // 6. Low-yield (R1.4). The recent job produced fewer than
