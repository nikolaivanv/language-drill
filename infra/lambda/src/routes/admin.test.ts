...
describe('GET /admin/pool-status', () => {
  ...
  it('returns 200 with all-zero counts for every curriculum cell on an empty DB', async () => {
    ...
    for (const item of body) {
      ...
      // all levels = 5 (2026-06-17); contextual_paraphrase B1/B2 = 8 (2026-07-08)).
      // Cells with a coverageSpec are raised to the largest single-axis floor
      // sum (Phase 2): ES A1/A2 person = 5×5 = 25, TR A1
      // person = 6×5 = 30, TR A2 person = 6×8 = 48, ES B1/B2 person = 5×15 = 75.
      // Assert it resolved to a known target.
      expect([5, 6, 8, 10, 15, 20, 25, 30, 48, 50, 75]).toContain(item.generationTarget);
