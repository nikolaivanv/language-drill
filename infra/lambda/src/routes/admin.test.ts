...
describe('GET /admin/pool-status', () => {
  ...
  it('returns 200 with all-zero counts for every curriculum cell on an empty DB', async () => {
    ...
    for (const item of body) {
      ...
      // at 10 every level; dictation A1=10, A2=12, B1/B2 = 15). Cells with a
      // coverageSpec are raised to the largest single-axis floor sum (Phase 2):
      // TR A1 person = 6×5 = 30, TR A2 person = 6×8 = 48, ES B1/B2 person =
      // 5×15 = 75. Assert it resolved to a known target.
      expect([10, 12, 15, 20, 30, 48, 50, 75]).toContain(item.generationTarget);
      expect(['ES', 'DE', 'TR']).toContain(item.language);
      expect(['A1', 'A2', 'B1', 'B2']).toContain(item.level);
      expect(['cloze', 'translation', 'vocab_recall', 'sentence_construction', 'dictation']).toContain(item.type);
