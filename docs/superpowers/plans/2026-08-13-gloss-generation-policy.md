...
- **Generate↔validate contract:** a generation-side rule is nullified if the validator still rejects the shape it produces. Task 1 and Task 2 must both land before the generator/validator contract is consistent in-repo — but note `eval:gen` cannot exercise the validator half at all: its `--baseline`/`--candidate` axis only swaps the generation prompt, and validation always goes through the LIVE Langfuse `validate-system-prompt` body (`buildValidationSystemPrompt` → `getPromptWithVarsOrFallback`), not the in-repo template. Closing that gap needs a post-merge `eval:gen` run against an environment that has received `push-prompts`.
