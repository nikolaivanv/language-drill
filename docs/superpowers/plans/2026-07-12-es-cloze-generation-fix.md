...
- Both edits live in **code** (tool schema + `buildGenerationUserPrompt`), not `GENERATION_SYSTEM_PROMPT_TEMPLATE`. Therefore **no `push-prompts` is required** and the [langfuse-registers-template-not-rendered-body] / [push-prompts-stale-worktree] traps do not apply. Do NOT edit the template in PR1.
