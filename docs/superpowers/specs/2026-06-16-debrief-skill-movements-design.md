...
- **Slip against a thin prior** → the panel welds the confidence cue to the band ("slipped · we're confident"), so the cue must describe the *delta*, which is only as good as its weaker end — hence `min(before, after)` rather than `after` alone. This matters because `updateMastery` seeds a first-ever row to its raw score: one perfect answer pins the point at 1.0 and every later movement can only be down. Observed in prod session `ec7dd00f` — five **correct** answers (header: "5 of 5 · 100%") replayed 1.0 → 0.927 and rendered "slipped · we're confident" off a single-sample baseline. The unearned-ceiling bias itself is left to the mastery model (out of scope below).
